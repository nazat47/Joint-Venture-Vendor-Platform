...
const ShopProfilePreview = ({ isOwner, previewShop }) => {
    ...
    const getEventAll = async () => {
      try {
        ...
        const { data } = await axios.get(
          `${REACT_APP_BASE_URL}/event/getAll/${previewShop?._id}`
        );
        if (data.msg) {
  ...
  return (
    <div className="w-full">
      ...
      {active === 2 && (
        <div className="w-full">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-1 md:gap-6 lg:grid-cols-1 lg:gap-6 xl:grid-cols-1 xl:gap-8 mb-12">
...
export default ShopProfilePreview;
