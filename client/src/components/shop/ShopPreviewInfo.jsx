...
const ShopPreviewInfo = ({ isOwner,previewShop }) => {
    ...
    const getProductAll = async () => {
      try {
        ...
        const { data } = await axios.get(
          `${REACT_APP_BASE_URL}/product/getAll/${previewShop?._id}`
        );
        if (data.msg) {
  ...
  return (
    <>
      ...
      <div className="p-3">
        ...
        <h4 className="text-gray-700">{previewShop?.address}</h4>
      </div>
      <div className="p-3">
        <h5 className="font-[600]">Phone number:</h5>
        <h4 className="text-gray-700">{previewShop?.phone}</h4>
      </div>
      <div className="p-3">
      ...
      <div className="p-3">
        ...
        <h4 className="text-gray-700">{previewShop?.createdAt.slice(0, 10)}</h4>
      </div>
      {isOwner && (
...
export default ShopPreviewInfo;
